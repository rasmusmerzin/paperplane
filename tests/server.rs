...
fn disconnect() -> WsResult<()> {
    task::block_on(async {
        ...
        assert_eq!(clients[1].next().await.unwrap()?, Message::Close(Some(CloseFrame {
            code: CloseCode::Normal,
            reason: "half".into(),
        })));
        assert_eq!(server.next().await.unwrap(), Event::Kicked(1, "half".into()));

        server.kick(2, "kick").await?;
        assert_eq!(clients[2].next().await.unwrap()?, Message::Close(Some(CloseFrame {
            code: CloseCode::Normal,
            reason: "kick".into(),
        })));
        ...
        assert_eq!(clients[3].next().await.unwrap()?, Message::Close(Some(CloseFrame {
            code: CloseCode::Normal,
            reason: "all".into(),
        })));
